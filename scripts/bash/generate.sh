...
parse_params() {
  ...
  while :; do
    ...
    -i | --input)
      input_file="${2-}"
      log_debug "parse_params input_file=$input_file"
      shift
      ;;
    -g | --generator)
      generator="${2-}"
      log_debug "parse_params generator=$generator"
      shift
      ;;
...
$OPENAPI_EXEC $args --type-mappings array=List $opts --global-property=debugModels &> logs/${command}_log.txt
